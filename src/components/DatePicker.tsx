            ...
            {visible && !disabled && (
                <div className="datepicker-popup" style={{borderColor: borderColor, borderRadius: `${radius}px`}}>
                  ...
                  <span onClick={() => setView('month')}
                        className={'datepicker-month-button'} style={{color:textColor}}>{persianMonths[currentMonth - 1]}</span>
                  <span style={{margin: '0 6px'}}/>
                  <span onClick={() => setView('year')} className={'datepicker-year-button'} style={{color:textColor}}> {currentYear}</span>
                </span>
                            ) : view === 'month' ? (
